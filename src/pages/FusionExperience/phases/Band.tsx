...
const Band = ({ setUIPhase, setGradient }: any) => {
  ...
  return (
    <>
      ...
      {type === "whoAreYou" && (
        <div
          className="flex gap-5 flex-col p-5 opacity-0 transition-opacity"
          ref={buttonsContainerRed}
        >
          <Button
            text="Profissional da saúde"
            onClick={() => onButtonClicked("professional")}
            style={{ border: "1px solid #f1e60e" }}
          />
          <Button
            text="Parceiro"
            onClick={() => onButtonClicked("owner")}
            style={{ border: "1px solid #742165" }}
          />
          <Button
            text="Dono de clínica"
            onClick={() => onButtonClicked("partner")}
